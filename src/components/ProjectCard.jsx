...
const ProjectCard = ({ project_name, description, tech, image, openCard }) => {
  return (
    <div className="group relative flex-1 min-h-96 max-w-full lg:max-w-[25%] overflow-hidden rounded-2xl bg-linear-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800 border border-slate-200 dark:border-slate-700 transition-all duration-300 hover:shadow-2xl hover:shadow-slate-300/50 dark:hover:shadow-slate-900/50 hover:-translate-y-1">
      <div className="relative h-48 overflow-hidden rounded-t-2xl bg-linear-to-br from-indigo-400 via-purple-400 to-pink-400">
        ...
        <div className="absolute inset-0 bg-linear-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end justify-center pb-4">
          <button className="px-4 py-2 cursor-pointer bg-white/90 dark:bg-slate-800/90 text-slate-900 dark:text-white rounded-full text-sm font-medium transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300 hover:bg-white dark:hover:bg-slate-700" onClick={openCard} >
            View Project
          </button>
        </div>
      </div>
